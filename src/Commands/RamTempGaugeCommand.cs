namespace Loupedeck.LLHMPlugin.Commands
{
    ...
    /// 게이지 범위 20~100°C. 45°C까지 초록, 55°C까지 노란색, 이상 빨간색.
    /// </summary>
    public class RamTempGaugeCommand : BaseSensorCommand
    {
        ...
        private void DrawGauge(BitmapBuilder builder, double temp)
        {
            ...
            var valueColor = temp >= 55 ? RedColor
                           : temp >= 45 ? YellowColor
                           : GreenColor;
        ...
        private void DrawThresholdArcs(BitmapBuilder builder, double cx, double cy, double outerR)
        {
            ...
            // 구간별로 호 그리기 (RAM: 20~44, 46~54, 56~100)
            DrawArcSegment(builder, cx, cy, arcRadius, thickness, 20, 44, GreenColor);
            DrawArcSegment(builder, cx, cy, arcRadius, thickness, 46, 54, YellowColor);
            DrawArcSegment(builder, cx, cy, arcRadius, thickness, 56, 100, RedColor);
        }
