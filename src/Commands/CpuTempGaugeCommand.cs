namespace Loupedeck.LLHMPlugin.Commands
{
    ...
    public class CpuTempGaugeCommand : BaseSensorCommand
    {
        ...
        private void DrawGauge(BitmapBuilder builder, double temp)
        {
            ...
            var valueColor = temp >= 90 ? RedColor
                           : temp >= 70 ? YellowColor
                           : GreenColor;
        ...
        private void DrawThresholdArcs(BitmapBuilder builder, double cx, double cy, double outerR)
        {
            ...
            // 구간별로 호 그리기 (CPU: 20~69, 71~89, 91~100)
            DrawArcSegment(builder, cx, cy, arcRadius, thickness, 20, 69, GreenColor);
            DrawArcSegment(builder, cx, cy, arcRadius, thickness, 71, 89, YellowColor);
            DrawArcSegment(builder, cx, cy, arcRadius, thickness, 91, 100, RedColor);
        }
