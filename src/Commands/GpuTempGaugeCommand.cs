namespace Loupedeck.LLHMPlugin.Commands
{
    ...
    public class GpuTempGaugeCommand : BaseSensorCommand
    {
        ...
        public GpuTempGaugeCommand()
            : base("GPU Temp (Gauge)", "GPU Temp Gauge View", "Hardware Monitor")
        {
        }
        ...
        {
            var gap = 2;              // arc와 구분 호 간격
            var thickness = 2;         // 구분 호 두께
            var arcRadius = outerR + gap + thickness / 2.0;

            // 구간별로 호 그리기 (각 구간 사이에 1도 gap)
            DrawArcSegment(builder, cx, cy, arcRadius, thickness, 20, 59, GreenColor);    // Green: 20~59°C
            DrawArcSegment(builder, cx, cy, arcRadius, thickness, 61, 74, YellowColor);   // Yellow: 61~74°C
            DrawArcSegment(builder, cx, cy, arcRadius, thickness, 76, 100, RedColor);     // Red: 76~100°C
        }
