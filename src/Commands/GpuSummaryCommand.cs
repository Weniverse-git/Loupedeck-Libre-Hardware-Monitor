namespace Loupedeck.LLHMPlugin.Commands
{
    ...
    /// <summary>
    /// GPU 사용량(%), 전력(W), 온도(°C)를 하나의 터치 버튼에 3줄로 통합 표시합니다.
    /// 각 줄은 해당 센서의 임계값에 따라 독립적으로 색상이 변합니다.
    /// </summary>
    public class GpuSummaryCommand : BaseSensorCommand
    {
        public GpuSummaryCommand()
            : base("GPU Summary", "GPU Load / Power / Temperature", "HW Monitor - GPU")
        {
        }
        ...
        protected override void DrawSensorData(BitmapBuilder builder, LhmDataService service)
        {
            ...
            var loadPath = registry?.GetGpuLoadPath();
            var powerPath = registry?.GetGpuPowerPath();
            var tempPath = registry?.GetGpuTemperaturePath();
            ...
            // 라벨
            builder.DrawText("GPU", 0, 2, builder.Width, 16,
            ...
            var powerColor = watts >= 0 ? GetGpuPowerColor(watts) : DisplayHelper.OfflineColor;
            builder.DrawText(powerText, 0, 34, builder.Width, 17,
                powerColor, 14);

            // 3줄: 온도 °C (GPU: 60/75 기준)
            var tempText = temp >= 0 ? $"{temp:F0}\u00B0C" : "N/A";
            var tempColor = temp >= 0 ? DisplayHelper.GetTemperatureColor(temp, 60, 75) : DisplayHelper.OfflineColor;
        ...
        /// <summary>
        /// GPU 전력 색상 (GpuPowerCommand와 동일 기준: 300W/500W)
        /// </summary>
        private static BitmapColor GetGpuPowerColor(double watts)
        {
            if (watts >= 500)
