namespace Loupedeck.LLHMPlugin.Commands
{
    ...
    public class GpuMemoryLoadBlockCommand : BaseSensorCommand
    {
        ...
        private static readonly BitmapColor BaseColor = new BitmapColor(180, 100, 255);
        private static readonly BitmapColor FillColor = new BitmapColor(255, 60, 60);

        public GpuMemoryLoadBlockCommand()
            : base("GPU VRAM (Block)", "GPU Memory Load Block View", "Hardware Monitor")
        {
        }
        ...
        protected override void DrawSensorData(BitmapBuilder builder, LhmDataService service)
        {
            ...
            var filledBlocks = (int)(percent / 5.0);
            filledBlocks = Math.Max(0, Math.Min(TotalBlocks, filledBlocks));

            builder.DrawText($"VRAM {percent:F0}%", 0, 2, builder.Width, 16,
